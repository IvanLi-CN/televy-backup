...
- `SnapshotInspector` resolves current snapshot filemaps, current two-level index caches, and legacy single-index snapshots. It calculates direct-baseline changes and distinct logical block pages with separate changed-file and total referencing-file counts, request-bound versioned cursors, and a changes-only block filter.
