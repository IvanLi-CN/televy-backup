...
- `backup.enqueue` 是 App 的全量、单目标和导入后合并本地目录入口；接收时会读取已原子写入的最新 settings，并在主循环采纳它之前阻止队列启动，随后对新增 target 重新投影现有 batch membership。CLI 仅负责将 scope 传给 control IPC，`backup run` 保持独立直跑语义。
