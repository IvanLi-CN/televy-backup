...
- 完整退出先 disable LaunchAgent 以阻止 keep-alive 重启，优雅停止后再 bootout；bootout 失败会取消 App 退出；remote index preflight 与 post-backup bootstrap update 均接入任务取消 token，CLI start 脱离调用终端。
