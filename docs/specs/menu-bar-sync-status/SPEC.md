...
- 菜单栏图标必须使用透明单色 template 品牌标记作为产品主体。Idle 使用无透明占位的原始 `18 × 18pt` 画布；Dev Idle 必须保留既有 `+ DEV` 图像构成，不得改色、缩放、拉伸或调整徽标位置。活动状态保持同一 `18 × 18pt` 画布，并在原图右下直接叠加 `11 × 11pt` 单色圆形状态徽标；徽标外仅在与主体相接的左侧和上侧扣出 `12 × 12pt` 透明圆形净空轮廓，右侧和下侧直接贴合图像边界。内部再扣出箭头、勾选或双向箭头。主体本身不得改色或变形。错误状态的主盘体继续保持 template 语义；同位置的右下警告标记只使用一种红色，感叹号以透明镂空呈现。
...
本次品牌替换仍使用同一受控预览路径，主体改为透明单色 template 品牌标记；活动、失败与 DEV 徽标语义保持不变。
