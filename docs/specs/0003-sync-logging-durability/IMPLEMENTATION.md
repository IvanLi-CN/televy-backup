...
- Filemap statements remain bounded to 512 rows, while the scan keeps one
  transaction open and commits it once after all file, chunk, and base-copy
  work succeeds. Base file metadata is looked up once per batch. Scan trace
  version 2 exposes the cumulative time and count of `files.insert`,
  `base.files.lookup`, `base_copy`, `chunks.insert.filemap`,
  `file_chunks.insert`, and `filemap.commit`, so a run can identify whether
  SQLite write, base lookup, chunk-copy, or final commit work dominates without
  logging file-level data. The temporary snapshot filemap uses a single-writer
  WAL with scan-time auto-checkpoint and sync disabled; it restores FULL sync and
  checkpoints before the SQLite file is uploaded.
