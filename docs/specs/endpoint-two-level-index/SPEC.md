# 端点索引二级拆分：Endpoint DB（一级）+ Snapshot Filemap DB（二级）+ 严格远端门禁（Fail Fast）
...
- 新读入文件的 `chunks`/`file_chunks` 以有界多值语句提交；未变化文件先一次播种
  基线 snapshot 的 chunk rows，再以集合映射写入 `file_chunks`，避免逐文件 DISTINCT
  查询与提交。
- 临时 filemap 连接使用单写入者 WAL、64 MiB cache、关闭自动 checkpoint 与扫描期 sync；
  index 上传前恢复 FULL sync 并执行 `wal_checkpoint(TRUNCATE)`。
