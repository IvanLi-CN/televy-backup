...
- If the retained-snapshot catalog or filemap cannot be read while serving a request, the service returns a server error and MUST NOT present a diagnostics-only or otherwise empty root as a successful listing.
