...
## `telegram_endpoints.rate_limit`（上传速率限制）
...
- 仅约束 **backup 的上传阶段**（调用 `Storage::upload_document`），且覆盖所有上传类型：
  - pack 上传
  - “直传 blob”（无法/不应打包时的单对象上传）
  - index 上传（index parts + manifest 等）
- 不约束 restore/verify 的下载（如未来需要，另立计划）。
...
- 最小间隔：**两次 upload 启动**之间的时间间隔不小于 `min_delay_ms`。
  - 若存在多个 worker，仍需保证“全局启动速率”满足该约束（按 endpoint 维度）。
