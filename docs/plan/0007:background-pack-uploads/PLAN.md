...
- `min_delay_ms` 按 endpoint 维度的“全局上传启动速率”执行（所有 worker 共享同一个节流器）。
...
- 以 `config.toml` 中现有的 `telegram_endpoints.rate_limit.*` 作为上传速率限制的来源（从“字段存在但不生效”变为“字段语义生效”）。
...
- 并发与速率限制必须可控：使用 `telegram_endpoints.rate_limit.max_concurrent_uploads` 与 `min_delay_ms` 来约束 upload worker 的行为。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `config.toml`：`telegram_endpoints.rate_limit.*` | Config | external | Modify | ./contracts/config.md | app/core | CLI / daemon / macOS app | 从“字段存在但不生效”变为“语义生效（上传并发/最小间隔）” |
...
- Given `telegram_endpoints.rate_limit.max_concurrent_uploads = 2`，
  When 执行一次 backup 且产生多个 pack 上传，
  Then 同时进行的上传任务数不超过 2（可通过测试用假 storage 统计并发峰值验证）。
- Given `telegram_endpoints.rate_limit.min_delay_ms = 250`，
  When 连续触发多次上传，
