# Telegram 通信升级为 MTProto API（替代/并存 Bot API）（#0004）
...
- 旧数据兼容：不要求兼容/恢复历史 Bot API 备份数据；当 snapshot/provider 不匹配时必须给出可操作错误提示（切回 `botapi` 或重新备份）。
...
- MTProto 与 Bot API 允许并存：通过配置选择通信模式；默认不破坏既有 Bot API 路径。
...
- SQLite index 兼容：
  ...
  - 不要求兼容读取既有 Bot API 备份数据；若 snapshot/provider 不匹配必须给出可操作提示（切回 `botapi` 或重新备份）。
- CLI：
...
- Daemon：
  - 按配置选择 `telegram.botapi` / `telegram.mtproto` 执行 scheduled backup（不改变既有调度语义）。
...
- 新增 MTProto 存储适配器：
  ...
  - 与现有 `telegram.botapi` 并存，通过 `telegram.mode` 选择（默认维持 `botapi`）。
- 鉴权与会话持久化：
  - 支持以 bot 身份登录；
  - 会话持久化（重启后可继续使用）；
...
- 对象引用格式与 DB 兼容：
  ...
  - 不支持在 `telegram.mode=mtproto` 下恢复历史 `telegram.botapi` snapshot（需给出可操作提示：切回 `botapi` 或重新备份）。
- 配置与验证：
...
| `televybackup telegram validate`（mtproto 端到端验证） | CLI | internal | Modify | `./contracts/cli.md` | CLI | GUI/User | Bot API 校验保持兼容 |
...
- `README.md`: 补充 MTProto 模式启用步骤、风险提示与回滚说明（切回 botapi）。

## 实现里程碑（Milestones）

- [ ] M1: 冻结具体 MTProto crate + session 持久化方案（并在契约中固化）
- [ ] M2: `telegram.mtproto` 基础连通（bot 登录 + upload_document + 小对象 download_document）
- [ ] M3: 大对象下载（分片/续传/重试 + 节流/并发控制 + 低内存峰值）
- [ ] M4: DB 口径与 `object_id` 形状落地（`tgmtproto:v1:` + provider 不匹配错误提示）
- [ ] M5: CLI validate + GUI 状态展示 + 文档与测试补齐

## 方案概述（Approach, high-level）

- 以 `Storage` 抽象为边界：MTProto 与 Bot API 仅在存储适配层分叉，备份语义与加密/索引逻辑保持不变。
