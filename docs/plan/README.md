...
| fh5ac | 修复上传速度显示不正确（MTProto progress + status stream） | 已完成 | `fh5ac:fix-upload-rate-display/PLAN.md` | 2026-02-13 | PR #38；待主人验收 |
| mycnc | 状态面板：下载速率实时显示与稳定性修复（1s window） | 已完成 | `mycnc:status-down-rate/PLAN.md` | 2026-02-19 | PR #43（已验收） |
| dxddw | Import bundle：Targets 增加更换目录按钮 | 已完成 | `dxddw:import-bundle-target-change-directory/PLAN.md` | 2026-02-07 | PR #35 |
| w2k9p | Import bundle：Rebind 目录一致性校验与冲突决策（本地/远端/合并） | 已完成 | `w2k9p:import-bundle-rebind-remote-compare/PLAN.md` | 2026-02-07 | PR #35；待主人验收 |
...
| 7bq4a | 发行版备份/验证稳定性修复（Telegram 超时、索引误判、瞬态文件、Vault Key 缓存） | 已完成 | `7bq4a:prod-backup-verify-stability/PLAN.md` | 2026-02-19 | PR #44 |
...
| 7r6p4 | 后台备份吞吐自适应优化（稳定优先，尽量跑满带宽） | 已完成 | `7r6p4:background-backup-adaptive-throughput/PLAN.md` | 2026-02-24 | PR #46；完整 30 分钟复测（opt9）`>=1 MiB/s` 累计 20.62 分钟，CI run #245 已绿 |
