...
| 0007 | Pack 上传后台并发（scan 与 upload 解耦） | 待实现 | `0007:background-pack-uploads/PLAN.md` | 2026-01-22 | 已冻结：队列双阈值（jobs+bytes）+ 等待回压；`min_delay_ms` 全局节流；限速来源 `telegram_endpoints[].rate_limit.*`；覆盖所有 upload（pack/直传/index） |
