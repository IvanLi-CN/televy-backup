...
- **APFS Snapshot Access**: `televybackup-snapshot-access` (`crates/snapshot-helper/`).
  ...
  - Official ad-hoc registration uses the fixed canonical `Program` path inside `/Applications/TelevyBackup.app` with `launchctl`; separately signed builds may use `SMAppService.agent` and `BundleProgram`. The CLI exposes status and a transactional migration from the old external registration. Scheduled backups use the already registered helper and never prompt for a password. Ordinary main-app releases reuse unchanged helper bytes.
