...
if [[ -z "$scene" || -z "$out" ]]; then
  ...
  echo "Scenes: settings-failure | targets-empty | endpoints-empty | targets-unselected | endpoints-unselected | schedule | snapshots | snapshots-disabled | snapshots-helper-missing | snapshots-mount-helper-missing | snapshots-fda-required | snapshots-cleanup-pending | service-installed | service-update | service-conflict | service-failure | diagnostics-normal | diagnostics-debug | diagnostics-override | diagnostics-retention" >&2
  exit 2
fi
