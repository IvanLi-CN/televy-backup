...
if [[ -z "$scene" || -z "$out" ]]; then
  ...
  echo "Scenes: main-window-targets | main-window-target-detail | main-window-target-connecting-queued | main-window-target-running-next-queued | main-window-target-starting | main-window-snapshot-changes | main-window-snapshot-baseline-unavailable | main-window-snapshot-storage | main-window-snapshot-failed-unavailable" >&2
  exit 2
fi
