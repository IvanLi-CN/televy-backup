...
mod tests {
    ...
    #[tokio::test]
    async fn root_directory_does_not_hide_catalog_errors_as_empty() {
        let temp = tempfile::tempdir().unwrap();
        let endpoint_db_path = temp.path().join("missing-endpoint.sqlite");
        let session = test_session(&endpoint_db_path, temp.path()).await;
        ...
        let response = respond(
            "PROPFIND",
            "/capability/",
            &HashMap::from([(String::from("depth"), String::from("1"))]),
            &[],
            &session,
        )
        .await;
