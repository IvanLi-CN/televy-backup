...
use serde_json::json;
use televybackup_snapshot_access::{DEFAULT_MOUNT_HELPER_PATH, mount_helper};

use super::CliError;

pub const MOUNT_HELPER_LABEL: &str = "com.ivan.televybackup.snapshot-mount-helper";
const MOUNT_HELPER_INSTALL_PATH: &str = DEFAULT_MOUNT_HELPER_PATH;
...
fn reject_if_busy(operation: &str) -> Result<(), CliError> {
    match mount_helper::status() {
        ...
        Ok(_) => Ok(()),
        Err(_error)
            if !safe_root_asset(Path::new(MOUNT_HELPER_INSTALL_PATH))
                && !safe_root_asset(Path::new(MOUNT_HELPER_PLIST_PATH)) =>
        {
            Ok(())
        }
        Err(error) => Err(CliError::retryable(
            "snapshot_mount_helper.unavailable",
            format!("cannot {operation} while helper status is unavailable: {error}"),
        )),
    }
}
