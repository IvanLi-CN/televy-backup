...
use crate::sender::{connect_with_auth_with_net_stats, connect_with_net_stats};
use crate::{InvocationError, NetStats, Sender, ServerAddr};
...
pub struct SenderPool {
    ...
    /// Cumulative `(bytes_in, bytes_out)` counters for all sender connections in this pool.
...
impl SenderPoolRunner {
    ...
    async fn connect_sender(
        ...
        let mut sender = if let Some(auth_key) = dc_option.auth_key {
            connect_with_auth_with_net_stats(transport(), addr(), auth_key, Arc::clone(&self.net_stats)).await?
        } else {
            connect_with_net_stats(transport(), addr(), Arc::clone(&self.net_stats)).await?
        };
        ...
        let enums::Config::Config(remote_config) = match sender.invoke(&init_connection).await {
            ...
            Err(InvocationError::Transport(transport::Error::BadStatus { status: 404 })) => {
                sender = connect_with_net_stats(transport(), addr(), Arc::clone(&self.net_stats)).await?;
