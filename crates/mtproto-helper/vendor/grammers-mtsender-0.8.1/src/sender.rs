...
impl<T: Transport, M: Mtp> Sender<T, M> {
    ...
    pub async fn connect(transport: T, mtp: M, addr: ServerAddr) -> Result<Self, io::Error> {
        Self::connect_with_net_stats(transport, mtp, addr, Arc::new(NetStats::default())).await
    }

    pub(crate) async fn connect_with_net_stats(
        transport: T,
        mtp: M,
        addr: ServerAddr,
        net_stats: Arc<NetStats>,
    ...
    fn on_net_read(&mut self, n: usize) -> Result<Vec<UpdatesLike>, ReadError> {
        ...
        self.net_stats.inc_in(n as u64);
        self.read_tail += n;
    ...
    fn on_net_write(&mut self, n: usize) {
        self.net_stats.inc_out(n as u64);
        self.write_head += n;
        trace!(
...
pub(crate) async fn connect_with_net_stats<T: Transport>(
    transport: T,
    addr: ServerAddr,
    net_stats: Arc<NetStats>,
) -> Result<Sender<T, mtp::Encrypted>, InvocationError> {
    let sender = Sender::connect_with_net_stats(transport, mtp::Plain::new(), addr, net_stats).await?;
    generate_auth_key(sender).await
}

pub(crate) async fn connect_with_auth_with_net_stats<T: Transport>(
    transport: T,
    addr: ServerAddr,
    auth_key: [u8; 256],
    net_stats: Arc<NetStats>,
) -> Result<Sender<T, mtp::Encrypted>, io::Error> {
    Sender::connect_with_net_stats(
        transport,
