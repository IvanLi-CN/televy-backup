...
pub const DEFAULT_MOUNT_HELPER_PATH: &str =
...
pub struct VerificationResult {
    pub volume_uuid: String,
    pub snapshot_read_pre_mutation: bool,
    pub live_mutation_observed: bool,
    pub leases_released: bool,
...
mod tests {
    ...
    #[test]
    fn verification_request_and_result_round_trip() {
        let request = Request {
            version: PROTOCOL_VERSION,
            request_id: "verify-1".to_string(),
            method: Method::VerifyTimepoint {
                target_id: "target-1".to_string(),
                confirm_probe_write: true,
            },
        };
        let encoded = serde_json::to_string(&request).unwrap();
        let decoded: Request = serde_json::from_str(&encoded).unwrap();
        ...
        let response = Response::ok(
            "verify-1",
            ResponseResult::Verification(VerificationResult {
                volume_uuid: "A8A38394-49AE-42F9-85FB-14DD9EA549FA".to_string(),
                snapshot_read_pre_mutation: true,
                live_mutation_observed: true,
        ...
        assert!(matches!(
            decoded.result,
            Some(ResponseResult::Verification(VerificationResult {
                snapshot_read_pre_mutation: true,
                cleanup_complete: true,
                ..
            }))
